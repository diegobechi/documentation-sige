...
/*devolver todas  las calificacion de un alumno en un ciclo lectivo*/ 

select asi.nombre as 'Asignatura' , ce.calificacion
...
select * from CalificacionEscolar ce
 where ce.legajoAlumno = 100047
...
SELECT cw.fecha,cw.comunicado, d.apellido, d.nombre
        ...
        SELECT CONVERT(CHAR(38),cw.fecha,121),cw.comunicado, d.apellido, d.nombre
    FROM ComunicadoWeb cw, Docente d, Curso c
    WHERE cw.legajoDocente = d.legajoDocente and
        cw.idCurso = c.idCurso and
          (cw.fecha between ' 01/01/2015 ' and ' 19/01/2015 ' )and
        c.idCurso = 9